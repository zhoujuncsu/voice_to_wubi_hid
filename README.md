...
2. **云端转写（SiliconFlow）**：使用 `openai` 官方库通过 OpenAI-compatible API 调用 SiliconFlow 语音识别  
...
2. **Cloud STT (SiliconFlow)**: uses the official `openai` SDK against SiliconFlow's OpenAI-compatible endpoint  
...
- `V2WH_SILICONFLOW_API_KEY`：SiliconFlow API Key（云端转写）
...
安全提示 / Security:
- 不要把 API Key 写进代码或日志；使用环境变量（例如 `V2WH_SILICONFLOW_API_KEY`）。
...
- `pyaudio`（采集音频）
- `RPi.GPIO`（GPIO 按钮）
- `openai`（SiliconFlow 云端识别，OpenAI-compatible）
...
仅运行 WAV 模式时通常不需要 `pyaudio/RPi.GPIO`，但若要做真实转写仍需要安装 `openai` 并提供 SiliconFlow API Key。
